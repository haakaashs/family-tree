...
func addPerson(n, g string) {
	...
	err := utils.Conn.Model(utils.Person{}).Create(&person)
	if err.Error != nil {
		helper.Add.Help()
		os.Exit(0)
	}
...
func addRelation(s string) {
	...
	err := utils.Conn.Model(utils.Relation{}).Create(&relation)
	if err.Error != nil {
		helper.Add.Help()
		os.Exit(0)
	}
...
func connectPersons(c map[string]string) {
	...
	var count int64
	err := utils.Conn.Model(&utils.Relationship{}).
	...
	err = utils.Conn.Model(utils.Relationship{}).Create(&relationship)
	if err.Error != nil {
		helper.Connect.Help()
		os.Exit(0)
	}
...
func getRelation() {
	...
	err := utils.Conn.Model(&utils.Person{}).Find(&person)
	if err.Error != nil {
		helper.Get.Help()
		os.Exit(0)
	}
